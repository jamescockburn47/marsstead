...
// CORDUROY STRIKE FIVE — THE FINAL LAW (2026-07-20, James, fifth
// verdict): anisotropy itself is dead in the fine bands. Fields and
// wandering directions still left coherent stripes inside every patch;
// stretched fbm IS elongated ridges, and on smooth country elongated
// ridges ARE corduroy, whatever gates them. The fine detail is now
// ISOTROPIC mottle only — nothing directional exists to stripe, at any
// scale, in any channel. (Directional character belongs solely to the
// dune FIELDS in the heightfield, which are sparse, masked and curved.)
// the detail height-field the normal tilt reads: metre-scale rubble over
// a longer ISOTROPIC undulation — same fbm family as sky and dust.
// rubbleW lets the caller fade the fine band with distance.
float marsDetailH(vec2 p, float rubbleW) {
  return fbm(p * 1.7) * rubbleW + fbm(p * 0.21 + 5.0) * 0.3;
}
...
if (uAlbedoAmp > 0.001) {
  ...
  // 2 m ISOTROPIC mottle — tone, never streaks (strike five)
  float aN = fbm(vMarsPos.xz * 0.42 + 23.0) - 0.5;
