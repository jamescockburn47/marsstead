// kiosk.js — the siblings' kiosk guard, ported from Moorstead via Saltstead:
// keep a stray two-finger swipe (or a small child) from accidentally leaving the
// game. We swallow the BROWSER's own chrome gestures — back/forward navigation,
// the right-click/long-press menu, text-selection + image drag — and take the
// page fullscreen on first interaction. None of this touches the game's own
// input (drag-to-look, the keys, the touch HUD); it only stops the browser from
// navigating away or popping its menus. Pairs with the `overscroll-behavior:
// none` + `touch-action: none` rules in index.html (those kill swipe-to-go-back
// / pull-to-refresh at the CSS layer; the history trap below is the
// belt-and-braces catch for browsers that still fire it).
...
export function installKiosk() {
  ...
  //    selection alone inside form fields (the title screen's name box needs it).
